...
- Image-to-video (I2V) and Wan 2.2 TI2V modes — v1 targets Wan 2.1 T2V only
- Other networks (DiT, SDXL) — Wan-only for v1; add after Wan pattern is proven
...
1. Per-block `block.forward = block_forward` (handles Wan2.1 T2V modulation + sCM `norm_temb`)
...
| Distillation | ModelOpt `mtd.convert(model, ..., ("kd_loss", ...))` wraps in `DistillationModel` | FastGen's `build_teacher()` — external frozen Wan checkpoint compared to student output via MSE |
