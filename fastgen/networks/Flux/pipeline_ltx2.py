...
from diffusers.loaders import FromSingleFileMixin, LTX2LoraLoaderMixin
...
class LTX2Pipeline(DiffusionPipeline, FromSingleFileMixin, LTX2LoraLoaderMixin):
    ...
    Args:
        ...
        vocoder ([`LTX2Vocoder`]):
            Vocoder to convert mel spectrograms to waveforms.
    """
    ...
    def __call__(
        ...
        max_sequence_length: int = 1024,
        generate_audio: bool = True,  # NEW: set False to skip all audio ops
    ):
        r"""
        ...
        Args:
            ...
            noise_scale (`float`, *optional*, defaults to `0.0`):
                Noise interpolation factor applied to latents before denoising.
            num_videos_per_prompt (`int`, *optional*, defaults to 1):
                The number of videos to generate per prompt.
            generator (`torch.Generator` or `list[torch.Generator]`, *optional*):
                Generator(s) for deterministic generation.
            latents (`torch.Tensor`, *optional*):
                Pre-generated video latents.
            audio_latents (`torch.Tensor`, *optional*):
                Pre-generated audio latents. Ignored when ``generate_audio=False``.
            ...
            decode_noise_scale (`float`, defaults to `None`):
                Noise scale at the decode timestep.
            output_type (`str`, *optional*, defaults to `"pil"`):
                Output format for the generated video.
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether to return a structured output.
            attention_kwargs (`dict`, *optional*):
                Extra kwargs passed to the attention processor.
            callback_on_step_end (`Callable`, *optional*):
                Callback called at the end of each denoising step.
            callback_on_step_end_tensor_inputs (`List`, *optional*, defaults to `["latents"]`):
                Tensor inputs for the step-end callback.
            max_sequence_length (`int`, *optional*, defaults to `1024`):
                Maximum sequence length for the prompt.
            generate_audio (`bool`, *optional*, defaults to `True`):
                Whether to generate audio alongside the video. Set to ``False`` to skip all audio
                operations (latent preparation, denoising, decoding). When ``False``, the returned
                ``audio`` value will be ``None``. Automatically forced to ``False`` when the
                transformer was built with ``audio_enabled=False``.

        Examples:

        Returns:
            [`~pipelines.ltx.LTX2PipelineOutput`] or `tuple`:
                If `return_dict` is `True`, [`~pipelines.ltx.LTX2PipelineOutput`] is returned,
                otherwise a `tuple` is returned where the first element is the generated video frames
                and the second element is the generated audio (or ``None``).
        """
        ...
        if latents is not None:
            if latents.ndim == 5:
                ...
                raise ValueError(
                    ...
                    "[batch_size, seq_len, num_features] or [batch_size, latent_dim, latent_frames, latent_height, latent_width]."
                )
        ...
        # --- Audio latents (only prepared when run_audio) ---
        ...
        if run_audio:
            if audio_latents is not None:
                if audio_latents.ndim == 4:
                    logger.info(
                        "Got audio_latents of shape [batch_size, num_channels, audio_length, mel_bins], "
                        "`audio_num_frames` will be inferred."
                    )
                    _, _, audio_num_frames, _ = audio_latents.shape
                elif audio_latents.ndim == 3:
                    logger.warning(
                        f"You have supplied packed `audio_latents` of shape {audio_latents.shape}, so the latent "
                        f"dims cannot be inferred. Make sure the supplied `num_frames` and `frame_rate` are correct."
                    )
                else:
                    raise ValueError(
                        f"Provided `audio_latents` tensor has shape {audio_latents.shape}, but the expected shape "
                        "is either [batch_size, seq_len, num_features] or [batch_size, num_channels, audio_length, mel_bins]."
                    )
        ...
        if run_audio:
            ...
            _, _ = retrieve_timesteps(audio_scheduler, num_inference_steps, device, timesteps, sigmas=sigmas, mu=mu)
        else:
        ...
        # 6. Prepare micro-conditions (RoPE coords)
        # Video coords — always computed
        ...
        # Audio coords — only computed when run_audio
        if run_audio:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Audio input (only when run_audio)
                if run_audio:
                ...
                with self.transformer.cache_context("cond_uncond"):
                    noise_pred_video, noise_pred_audio = self.transformer(
                        ...
                        audio_encoder_hidden_states=connector_audio_prompt_embeds if run_audio else None,
                        timestep=timestep,
                        encoder_attention_mask=connector_attention_mask,
                        audio_encoder_attention_mask=connector_attention_mask if run_audio else None,
                        num_frames=latent_num_frames,
                        height=latent_height,
                        width=latent_width,
                        fps=frame_rate,
                        audio_num_frames=audio_num_frames if run_audio else None,
                ...
                # Audio denoising step — only when run_audio
                if run_audio:
        ...
        # 9. Decode audio (only when run_audio)
        if run_audio:
        ...
        if output_type == "latent":
            video = latents
            audio = audio_latents if run_audio else None
        else:
